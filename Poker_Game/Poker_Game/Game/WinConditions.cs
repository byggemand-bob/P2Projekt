...
namespace Poker_Game.Game {
    ...
    public class WinConditions {
        ...
        public Player SameScore(Player player1, Player player2) {
            ...
            if(player1.Score == Score.RoyalFlush) return null;

            if(player1.Score == Score.StraightFlush) return BestStraight(player1, player2);
            if(player1.Score == Score.FourOfAKind) return BestFourOfAKind(player1, player2);
            if(player1.Score == Score.FullHouse) return BestFullHouse(player1, player2);
            if(player1.Score == Score.Flush) return BestFlush(player1, player2);
            if(player1.Score == Score.Straight) return BestStraight(player1, player2);
            if(player1.Score == Score.ThreeOfAKind) return BestThreeOfAKind(player1, player2);
            if(player1.Score == Score.TwoPairs) return BestTwoPairs(player1, player2);
            if(player1.Score == Score.Pair) return BestPair(player1, player2);
            return null;
        }
        ...
        private int GetBestHighestCard(List<Card> cards1, List<Card> cards2) {
            ...
            while(i >= 0 && j >= 0) {
                if(cards1[i].Rank != cards2[j].Rank) return cards1[i].Rank < cards2[j].Rank ? 0 : 1;
                i--;
                j--;
            }
                        ...
                        if(player2Cards[j].Rank == player2Cards[j + 1].Rank &&
                           ...
                           player2Cards[j + 2].Rank == player2Cards[j + 3].Rank) {
                            if(player1Cards[i].Rank == player2Cards[j].Rank) {
                                ...
                                if(GetBestHighestCard(player1Cards, player2Cards) == 1) return player2;

                                if(GetBestHighestCard(player1Cards, player2Cards) == 0) return player1;
                                return null;
                            }
        ...
        private List<Card> GetXAmountOfHighestCard(List<Card> cards, int numberOfCards) {
            for(int i = cards.Count - numberOfCards - 1; i > 0; i--) cards.Remove(cards[i]);
            return cards;
        }
        ...
        private Player BestFullHouse(Player player1, Player player2) {
            ...
            player1Cards.Sort();
            player2Cards.Sort();
            if(BestThreeOfAKind(player1, player2) != null) return BestThreeOfAKind(player1, player2);
                                        ...
                                        for(int k = 0; k < player2.Cards.Count - 1; k++)
                                            if(player2.Cards[k].Rank == player2.Cards[k + 1].Rank) {
                                                if(player1.Cards[h].Rank == player2.Cards[k].Rank) {
                                                    Player player1Clone = (Player) player1.Clone();
                                                    Player player2Clone = (Player) player2.Clone();
                                                    player1Clone.Cards = RemoveUnfitRank(player1Clone.Cards,
                                                        player1Clone.Cards[i].Rank);
                                                    player2Clone.Cards = RemoveUnfitRank(player2Clone.Cards,
                                                        player2Clone.Cards[j].Rank);
                                                    player1Clone.Cards = RemoveUnfitRank(player1Clone.Cards,
                                                        player1Clone.Cards[h].Rank);
                                                    player2Clone.Cards = RemoveUnfitRank(player2Clone.Cards,
                                                        player2Clone.Cards[k].Rank);
                                                    ...
                                                    return GetBestHighestCard(player1Clone,
                                                        player2Clone); // Should check for highestCard here.
                                                }

                                                return player1.Cards[h].Rank > player2.Cards[k].Rank
                                                    ? player1
                                                    : player2;
                                            }
        ...
        private Player BestPair(Player player1, Player player2) {
            ...
            throw new InvalidOperationException("BestPair exited loop");
        }
