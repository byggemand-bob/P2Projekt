    ...
    public class FastWinCalc {
            ...
            //return -1 for ai win, 1 for player win, 0 for draw
        {
            ...
            if(aiEvalCards.HasFlush) {
                if(playerEvalCards.HasFlush) {
                    ...
                    if(aiEvalCards.HighestCardInStraight > 0) {
                        ...
                        if(aiHighestCardInStraightFlush > 0) {
                            ...
                            if(playerHighestCardInStraightFlush > aiHighestCardInStraightFlush)
                                return 1;
                            if(playerHighestCardInStraightFlush == aiHighestCardInStraightFlush) return 0;
                        }
                    }
                    ...
                    if(playerEvalCards.HighestCardInStraight > 0) {
                        ...
                        if(playerHighestCardInStraightFlush > 0) return 1;
                    }
                ...
                if(playerEvalCards.NrOfHighestCard >= 3) {
                    if(playerEvalCards.NrOfHighestCard == 4)
                        return 1;
                    if(playerEvalCards.NrOfSecondHighestCard >= 2) return 1;
                }
            ...
            if(playerEvalCards.HasFlush) {
                ...
                if(aiEvalCards.NrOfHighestCard == 4) return -1;

                //check for highest full house
                if(aiEvalCards.NrOfSecondHighestCard >= 2 && aiEvalCards.NrOfHighestCard == 3) return -1;
            ...
            if(aiEvalCards.NrOfHighestCard == 4) {
                if(playerEvalCards.NrOfHighestCard == 4) {
                    if(playerEvalCards.ValueOfHighestCard > aiEvalCards.ValueOfHighestCard)
                        return 1;
                    if(playerEvalCards.ValueOfHighestCard < aiEvalCards.ValueOfHighestCard) return -1;

                    return WhoHasHighCard(aiCards, playerCards, aiEvalCards.NrOfHighestCard,
                        aiEvalCards.NrOfSecondHighestCard, aiEvalCards.ValueOfHighestCard,
                        aiEvalCards.ValueOfSecondHighestCard);
                }

                return -1;
            }

            if(playerEvalCards.NrOfHighestCard == 4) return 1;
            ...
            if(aiEvalCards.NrOfSecondHighestCard >= 2 && aiEvalCards.NrOfHighestCard == 3) {
                if(playerEvalCards.NrOfSecondHighestCard >= 2 && playerEvalCards.NrOfHighestCard == 3) {
                    ...
                    if(playerEvalCards.ValueOfHighestCard < aiEvalCards.ValueOfHighestCard)
                        return -1;
                    if(playerEvalCards.ValueOfSecondHighestCard > aiEvalCards.ValueOfSecondHighestCard)
                        return 1;
                    if(playerEvalCards.ValueOfSecondHighestCard < aiEvalCards.ValueOfSecondHighestCard) return -1;
                    return 0;
                }
            ...
            if(playerEvalCards.NrOfSecondHighestCard >= 2 && playerEvalCards.NrOfHighestCard == 3) return 1;
            ...
            if(aiEvalCards.HighestCardInStraight > 0 || playerEvalCards.HighestCardInStraight > 0) {
                if(aiEvalCards.HighestCardInStraight > playerEvalCards.HighestCardInStraight)
                    return -1;
                if(aiEvalCards.HighestCardInStraight < playerEvalCards.HighestCardInStraight) return 1;
                return 0;
            }
            ...
            if(playerEvalCards.NrOfHighestCard < aiEvalCards.NrOfHighestCard)
                return -1;
            if(playerEvalCards.NrOfSecondHighestCard > aiEvalCards.NrOfSecondHighestCard)
                return 1;
            if(playerEvalCards.NrOfSecondHighestCard < aiEvalCards.NrOfSecondHighestCard)
                return -1;
        ...
        private int WhoHasHighCard(List<Card> aiCards, List<Card> playerCards, int nrOfHighestValueCards,
            ...
            if(nrOfSecoundHighestValueCards > 1 && nrOfHighestValueCards != 4) {
                ...
                for(int x = 0;
                    x < y;
                    x++) //checks for who has highest card, not incluling cards involved in a pair, three of a kind etc.
                {
                    while((int) playerCards[playerTestCard - x].Rank == valueOfHighestCard ||
                          (int) playerCards[playerTestCard - x].Rank == valueOfSecoundHighestCard) playerTestCard--;

                    while((int) aiCards[aiTestCard - x].Rank == valueOfHighestCard ||
                          (int) aiCards[aiTestCard - x].Rank == valueOfSecoundHighestCard) aiTestCard--;


                    if(aiCards[aiTestCard - x].Rank > playerCards[playerTestCard - x].Rank)
                        return -1;
                    if(aiCards[aiTestCard - x].Rank < playerCards[playerTestCard - x].Rank) return 1;
                }
                ...
                for(int x = 0;
                    x < y;
                    x++) //checks for who has highest card, not incluling cards in involved in a pair, three of a kind etc.
                {
                    while((int) playerCards[playerTestCard - x].Rank == valueOfHighestCard) playerTestCard--;

                    while((int) aiCards[aiTestCard - x].Rank == valueOfHighestCard) aiTestCard--;
                ...
                for(int x = 0;
                    ...
                    if(aiCards[aiTestCard - x].Rank > playerCards[playerTestCard - x].Rank)
                        return -1;
                    else if(aiCards[aiTestCard - x].Rank < playerCards[playerTestCard - x].Rank) return 1;
            }
        ...
        public int HasStraightFlush(List<Card> cards, Suit flushSuit) {
            ...
            if(cards.Count != 7) throw new Exception("Not enough cards");
            ...
            for(x = 0; x <= 6; x++)
                if(cards[x].Suit == flushSuit) {
                    if(cards[x].Rank == lastRank + 1) {
                        ...
                        if(consecutiveCardsOfSameSuitAndRank >= 5) result = (int) cards[x].Rank;
                    } else {
                        consecutiveCardsOfSameSuitAndRank = 1;
                        lastRank = cards[x].Rank;
                    }
        ...
        private int CompareFlushes(List<Card> player1Cards, Suit player1FlushSuit, List<Card> player2Cards,
                Suit player2FlushSuit)
            //returns -1 if player1 won, 1 if player 2 or 0 if a draw
        {
            ...
            for(int x = 0; x < 5; x++) {
                while(player1Cards[i].Suit != player1FlushSuit) i--;

                while(player2Cards[n].Suit != player2FlushSuit) n--;

                if(player1Cards[i].Rank > player2Cards[n].Rank)
                    return -1;
                if(player1Cards[i].Rank < player2Cards[n].Rank) return 1;
        ...
        public struct EvaluatedCards {
            ...
            public int NrOfHighestCard,
                ValueOfHighestCard,
                NrOfSecondHighestCard,
                ValueOfSecondHighestCard,
            ...
            private void EvaluationCalc(List<Card> cards) {
                ...
                if(cardsSummed[14] > 0) cardsInRow++;
                ...
                for(int x = 2; x < 15; x++)
                    if(cardsSummed[x] > 0) {
                        ...
                        if(cardsInRow >= 5) HighestCardInStraight = x;
                    } else {
                        cardsInRow = 0;
                    }
            }
