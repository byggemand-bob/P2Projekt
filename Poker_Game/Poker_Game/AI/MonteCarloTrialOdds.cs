...
{
    class MonteCarloTrialOdds
    {
        readonly Random _rndNr = new Random();
        private List<Card> _aiHand, _street;
        public int Wins = 0, Loses = 0, Draws = 0;

        readonly int _numberOfThreads = Environment.ProcessorCount;
        ...
        public Odds MultiThreadMonteCarlo(List<Card> hand, List<Card> street, List<List<Card>> range)
        {
            ...
            for (x = 0; x < _numberOfThreads; x++)
            {
                workers[x] = new Thread(() => { trialResults[x] = RunTrials(TotalNumberOfTrials / _numberOfThreads, range); });
                workers[x].Start();
                Thread.Sleep(100);
            }
        ...
        public Odds TestMultiThreadMonteCarlo(List<Card> hand, List<Card> street)
        {
            ...
            for (x = 0; x < _numberOfThreads; x++)
            {
                workers[x] = new Thread(() => { trialResults[x] = TestRunTrials(TotalNumberOfTrials / _numberOfThreads); });
                workers[x].Start();
                Thread.Sleep(100);
            }
